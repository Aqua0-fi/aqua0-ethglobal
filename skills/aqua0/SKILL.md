...
- **Graph errors or missing Aqua entities.** Graph tools surface errors and never fall back to RPC. Studio still serves the earlier schema, without `AquaStrategy`/`AquaFill`, so read venue state with `get_shared_backing` for now.

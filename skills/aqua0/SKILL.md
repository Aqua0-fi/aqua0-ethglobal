...
- **Honest status.** Pegged strategies and swaps are live on Arc Testnet. The forex router and adapter are deployed on Arc but the adapter is not wired yet, so forex strategies run on a fork. Say which one you used. Do not call ARGt or BRAt real pesos or reais.

## Troubleshooting

- **`opcodeNote` says forex was not used.** The forex adapter is not yet allowlisted or lacks `VENUE_SETTLER_ROLE` in the vault core, or the signer lacks `OPERATOR_ROLE` on it, so `create_strategy` fell back to a pegged strategy. Tell the user the strategy is fixed-price. Passing `opcode:"forex"` insists on the forex curve and fails until the core admin wires the adapter.
- **Old CryptoSwap params rejected.** `a`, `gamma`, `outFeeBps`, `feeGamma` and `flatFeeBps` belonged to the retired FXSwap curve and now return an error. Use `alpha`, `beta`, `delta`, `maxFee` (or `maxFeePercent`), `lambda`, and `feeBps` for the forex curve's fee. `maxFee` must stay below min(0.5, (1 − alpha) / (2 alpha)), or the strategy is rejected.

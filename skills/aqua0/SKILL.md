...
- **Honest status.** Pegged strategies and swaps are live on Arc Testnet. The forex router and adapter await deployment and wiring on Arc, so forex strategies run on a fork. Say which one you used. Do not call ARGt or BRAt real pesos or reais.
...
- **Old CryptoSwap params rejected.** `a`, `gamma`, `outFeeBps`, `feeGamma` and `flatFeeBps` belonged to the retired FXSwap curve. Use `alpha`, `beta`, `delta`, `maxFee`, `lambda`, and `feeBps` for the forex curve's fee.
...
- **"No live strategy" on forex.** No forex strategies exist on Arc until the forex router and adapter are deployed and wired. Use `opcode:"pegged"`, or a fork.

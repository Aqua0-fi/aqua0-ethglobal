...
**1. Claude Code plugin (easiest).** One install gives you this skill and the hosted MCP server, already connected. The hosted server runs the current build in prepare mode and never signs: 25 tools, Graph data from Subgraph Studio, and write tools that return calldata.
...
| Check book health and the keeper | "is the book healthy?" → `get_signals {pair?}`; "did the keeper rebalance?" / "show the keeper's spend" → `keeper_status {last?}` | Both read-only. `get_signals` reads the same oracle and book signals the keeper buys (spread near 30 bps is flat; above 150 bps the keeper rebalances). `keeper_status` reads the keeper's journal on the machine running the server, so use a local server next to the keeper: report `summary`, `lastRebalance` (spreads before and after, Arcscan links) and `spend`. |
...
| "is a 30bps euro strat even competitive onchain?" | `benchmark_fx_strategy {"pair":"USDC/EUR"}` |
| "how deep is onchain brl liquidity, last 30 days, 5 bps fee" | `benchmark_fx_strategy {"pair":"BRL","feeBps":5,"lookbackDays":30}` |
| "any real peso liquidity on polygon?" | `benchmark_fx_strategy {"pair":"ARS","chains":["polygon"]}` |
...
- **Keys.** Never ask for a private key and never accept one pasted into chat. If one is pasted, tell the user to treat it as compromised. Signing keys belong only in a local server's `WRITE_PRIVATE_KEY`, and should be throwaway testnet keys. With `SIGNER=circle`, the server signs through a Circle developer-controlled wallet instead. Its `CIRCLE_API_KEY` and `CIRCLE_ENTITY_SECRET` follow the same rule. The server checks every Circle EIP-712 signature against the wallet address. The public endpoint holds no key. `GRAPH_GATEWAY_API_KEY` is a secret too: it stays in the server environment, and the tool never returns it.
- **Benchmark honesty.** `benchmark_fx_strategy` counts only the sources that answered. Report its numbers as returned, say which pools come from the non-standardized fallback, and treat Arc Testnet fills as demo-token activity, not market volume. It does not evaluate the forex curve; for an exact Aqua0 price use `quote_swap`.
- **Honest status.** Forex and pegged strategies and swaps are live on Arc Testnet. Live swaps have filled inside the flat band; fills past it and the halt band have been shown on a fork. Say which venue you used. Do not call ARGt or BRAt real pesos or reais.
...
- **Curve params rejected.** `a`, `gamma`, `outFeeBps`, `feeGamma` and `flatFeeBps` are not forex curve params and return an error. Use `alpha`, `beta`, `delta`, `maxFee` (or `maxFeePercent`), `lambda`, and `feeBps` for the forex curve's fee. `maxFee` must be below 0.5 (so each quote has one solution), or the strategy is rejected.
...
- **Graph errors or missing Aqua entities.** Graph tools surface errors and never fall back to RPC. The live Studio version (`ethglobal-arc-3d0b9ef`) indexes both Aqua venues, pegged and forex: `AquaStrategy`, `AquaOrder` and `AquaFill` (swap fills), plus the AquaAdapter lifecycle events `aquaStrategyShippedEvents`, `aquaStrategyDockedEvents`, `aquaStrategyReshippedEvents` and `aquaStrategyReconciledEvents`. For backing use `get_shared_backing`; for pricing use `quote_swap`. `feesCredited` on an `AquaFill` is a 1-wei remainder, because the adapter books the curve's spread as swap proceeds; LPs still receive the full input, so report `effectiveSpreadBps` for the real cost.

- **Benchmark source failed or partial.** The gateway sometimes answers "bad indexers" or times out; the tool retries once and then reports the source. Run it again later, or pass `chains` to skip a slow chain. `realizedFeeBps: null` means the subgraph does not price that pool in USD; the volume is still real (its USDC leg). "GRAPH_GATEWAY_API_KEY is required" means the server has no gateway key.

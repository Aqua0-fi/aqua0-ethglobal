...
| Item | Notes |
| --- | --- |
| Shape-C / AssetVault shared-capital contracts: VaultRegistry, VaultFactory, Composer, FillerRegistry, AssetVault | Aqua0 contracts repository. The Arc deployment used source commit `8a9f1c2`. |
...
| Base mainnet Shape-C deployment | Addresses in `packages/subgraph/subgraph.base.yaml` |
...
| --- | --- | --- |
| Shape-C subgraph schema + mappings, canonical event ABI indexing, required-events regression check | `packages/subgraph` | Live (self-hosted) |
...
| Arc Testnet deployment of the Shape-C core and USDC/ARGt/BRAt vaults | `deployments/arc-testnet.json` | Live |

...
| --- | --- | --- | --- |
| Agent interface | MCP server, 24 tools in execute mode (stdio + Streamable HTTP) | [`apps/mcp`](../apps/mcp) | **Live**. Public endpoint runs the current prepare-only build with 23 tools and no signer; `authorize_strategy` is execute-only. |

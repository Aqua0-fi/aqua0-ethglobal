...
The deploy job then checks both public health endpoints and lists the public MCP tools. `create_strategy`, `quote_swap`, and `benchmark_fx_strategy` are all required by the post-deploy smoke check.
...
- `GRAPH_GATEWAY_API_KEY` (required by the live `benchmark_fx_strategy` tool)

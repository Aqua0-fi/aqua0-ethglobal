...
| --- | --- | --- |
| Arc (chain `5042002`) | Yes | The Aqua0 vault core, three AssetVaults and the pegged 1inch Aqua venue are **Live**, with two strategies shipped and filled. RedStone BRL and MXNe price feeds are **Live**. The forex venue is **Deployed, awaiting wiring**. See [`ARC_DEPLOYMENT.md`](ARC_DEPLOYMENT.md). |
...
- **Status:** forex USDC/BRL through the MCP on these feeds is **Fork-proven**. The forex venue on Arc itself is **Deployed, awaiting wiring**.
...
| Functional MVP: backend | Dashboard Node API, MCP server ([`apps/mcp`](../apps/mcp)), typed service ([`packages/shared`](../packages/shared)), subgraph on Subgraph Studio, Arc contracts | **Live**; forex venue **Deployed, awaiting wiring** |
...
- **Fork-proven:** the forex-curve flow, including a RedStone price push before the USDC/BRL swap, the flat band, inventory fee and halt band, and an ARS feed move and re-quote ([`scripts/test-arc-fork-forex.sh`](../scripts/test-arc-fork-forex.sh)).

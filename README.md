...
- [x] **An autonomous agent that transacts: the FX book keeper.** `aqua0 keeper run` watches swaps on the live forex strategies, buys oracle and book signals, and rebalances a tilted book from its own Circle wallet with no human in the loop. An OpenAI model (`gpt-5-nano`) picks one action from a closed set; budget, max trade size, cooldown, allowed actions and dry-run are enforced in code, and a rules policy is the fallback. Live run: a 0.1 USDC swap tilted USDC/BRL to 265.53 bps and the keeper brought it back to 29.99 bps ([rebalance swap](https://testnet.arcscan.app/tx/0xa3a786f86937bb03ab6f5bc6dbb5745b7f22ab3cbf03a8dca164362a4ea165e4)). [Two-session demo](docs/DEMO.md#two-session-demo-autonomous-keeper) · **Live**
- [x] **Nanopayments, App Kit, Circle Wallets and ERC-8004.** The keeper pays per signal ($0.0005 to $0.001) through Circle Gateway batched x402 on Arc Testnet, signed by its Circle developer-controlled wallet; App Kit `send` funded that wallet; the keeper is ERC-8004 agent 894559 and received feedback after its rebalance. · **Live** (`keeperRun` in [`arc-testnet-strategies.json`](deployments/arc-testnet-strategies.json))
- [ ] **Paymaster and the Agent Stack starter kits.** Paymaster is not on Arc; the starter kits were not used. The keeper recommends docking a strategy with a stale oracle but cannot dock it (that needs the strategist's signature). · **Not used**

> [!WARNING]
> **Gaps / next:** the keeper and its signals seller run locally, not hosted. Circle wallet spending caps are mainnet-only, so the keeper's limits live in its executor. Details in [`docs/ARC_TRACK.md`](docs/ARC_TRACK.md#autonomous-fx-book-keeper).
